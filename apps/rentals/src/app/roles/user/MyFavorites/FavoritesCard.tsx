...
import { updateRMTask, deleteRMTask } from "../../../shared/services/api/index";
import tailwindStyles from "@packages/styles/tailwindStyles";
import PaymentModal from "../../../../../../../packages/ui/Payment/PaymentModal";
import BrokerageView from "./BrokerageView";
import { handlePayment } from "../../../../../../../packages/ui/Payment/PaymentUtils";
import AmenitiesList from "../../../features/Properties/AmentiesList";
...
const InfoItem: React.FC<InfoItemProps> = ({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-2 py-2 sm:px-3 sm:py-3 bg-gradient-to-br 
      from-gray-50 to-white hover:from-gray-100 hover:to-gray-50 
...
const FavouritesCard: React.FC<FavouritesCardProps> = ({
  ...
  const mapBackendDataToComponent = (
    property: Property
  ): MappedPropertyData => {
    const allImages = [
    ...
    return {
      ...
      communitydetails: `${property.total_area} Campus with ${property.totflats} Units ${
        property.majorArea ? ", " + property.majorArea : ""
      }`,
      location: property.address,
      details: {
        "Rent/Month": `₹ ${property.rental_high.toLocaleString()} ${
          property.maintenance_type === "Included" ? "+ Maintenance" : ""
        }`,
        ...
        Floor: property.floor_no?.toString() || "N/A",
        Parking: property.parking_count?.toString() || "N/A",
        Availability: property.available_date?.toString() || "N/A",
      },
    };
  };
  ...
  const [amenitiesData] = useState<any>(null);
  const [isAmenitiesLoading] = useState(false);
  ...
  const matchedProperty = userProperties.find(
    (u: any) => u.prop_id === property.prop_id
  );
  ...
  const connectToRM = async (propertyId: number) => {
    try {
      const result = await updateRMTask(userData.id, propertyId, 4);
      if (result.status === 200) {
        fetchActionsListings(userData.id);
      } else {
        throw new Error("Failed to connect RM");
      }
    } catch (error) {
      alert("Failed to connect Relationship Manager");
    }
  };
  ...
  const onClickRemoveFavourite = (event: MouseEvent<HTMLButtonElement>) => {
  ...
  const confirmRemoveFavourite = async (trId?: number) => {
    if (!trId) return;
...
const styles = `
  ...
  .animate-favorite { animation: pulseAndFill 0.4s ease-in-out; }

  @keyframes floatHeart {
    0% { transform: translateY(0); opacity: 1; }
    100% { transform: translateY(-100px); opacity: 0; }
  }
  .animate-float-heart { animation: floatHeart 1s ease-out forwards; position: fixed; z-index: 10; }

  @keyframes slideIn {
    0% { transform: translateX(-100%); opacity: 0; }
    100% { transform: translateX(0); opacity: 1; }
  }
  .animate-rm-slide { animation: slideIn 0.5s ease-out forwards; }
`;
