...
import { fetchPropertiesApi } from "@/app/shared/services/api/crud/adminCrud";
...
interface PropertyDetails {
  ...
  open_area?: number;
  nblocks?: number;
  nfloors_per_block?: number;
  nhouses_per_floor?: number;
  totflats?: number;
  nbaths?: number;
  nbalcony?: number;
  eat_pref?: string;
...
const PropertyDetailsModal = ({
  ...
  useEffect(() => {
    const fetchPropertyDetails = async () => {
      ...
      try {
        const response = (await fetchPropertiesApi({ property_id: propertyId })) as FetchPropertiesResponse;
        console.log("property data: ", response);

        if (response?.result) {
          setPropertyDetails(response.result);
        } else if (response?.results?.length) {
          const property = response.results.find((item) => item.id === propertyId);
          if (property) {
