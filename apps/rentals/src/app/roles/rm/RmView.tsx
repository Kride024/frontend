...
import { getTasks, getFmList, updateTask } from "../../shared/services/api/crud/rmCrud";
...
const RMView: React.FC = () => {
  const { userData } = useRoleStore();
  const { id } = userData;

  const [selectedCommunityId, setSelectedCommunityId] = useState<string>("");
  const [requestRmDetails, setRequestRmDetails] = useState<RmRequest[]>([]);
  const [rmCommunities, setRmCommunities] = useState<string[]>([]);
  const [rmStatuses, setRmStatuses] = useState<StatusOption[]>([]);
  const [fieldManagers, setFieldManagers] = useState<FieldManager[]>([]);
  const [expandedProperty, setExpandedProperty] = useState<number | null>(null);

  const [propDetails, setPropertyDetails] = useState<PropertyDetails | null>(
    null
  );
  ...
  const onClickPropertyId = async (propId: number) => {
    if (expandedProperty === propId) {
      setExpandedProperty(null);
      return;
    }
    try {
      const response = await axios.get(`${apiUrl}/adminPropListings?property_id=${propId}`);
      const details: PropertyDetails = response.data.results[0];
  ...
  useEffect(() => {
    const fetchRmDashboardData = async () => {
      ...
      try {
        ...
        const result: RmRequest[] = responseData?.result || [];

        setRequestRmDetails(result);
        setRmCommunities(result.map((i) => i.community_name).filter(Boolean));
  ...
  // -------------------------------------------------
  // FETCH FIELD MANAGERS ON COMMUNITY CHANGE
  // -------------------------------------------------

  useEffect(() => {
    if (!rmCommunities.length || !requestRmDetails.length) return;

    const fetchFmData = async () => {
      const currentCommId =
        requestRmDetails[0]?.community_id ?? 0;
  ...
  const handleSave = async (transactionId: number) => {
    ...
    try {
      const payload = {
        ...
        fm_id: updatedRequest.fm_id ? Number(updatedRequest.fm_id) : null,
        Inv_Amount: rentAmount ? Number(rentAmount) : undefined,
      };
  ...
  const filteredRequests = selectedCommunityId
    ? requestRmDetails.filter(
        (item) => item.community_id === Number(selectedCommunityId)
      )
    : requestRmDetails;

  const uniqueCommunities: Community[] = Array.from(
    new Set(requestRmDetails.map((item) => item.community_id))
  ).map((cid) => {
    const c = requestRmDetails.find((i) => i.community_id === cid);
    return {
      community_id: cid,
  ...
  return (
    <div className="bg-gray-200 min-h-screen flex flex-col">
      ...
      >
        ...
        <h1 className="text-lg hidden md:flex md:text-xl font-bold">RM Dashboard</h1>
