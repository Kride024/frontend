...
interface FilterSectionProps {
  currentPageChange: (filters: Record<string, any>) => void;
...
const FilterSection: React.FC<FilterSectionProps> = ({
  ...
  const [filters, setFilters] = useState<Record<string, any>>(() => ({
    ...
    builders: (searchParams.get("builders") as string) || "",
    ...
    tenanttype: searchParams.get("tenanttype")?.split(",").filter(Boolean) || [],
  }));
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFilters((prev) => {
      if (type === "checkbox") {
        ...
        return {
          ...prev,
          [name]: checked ? [...currentValues, value] : currentValues.filter((item: any) => item !== value),
        };
      }
      return {
        ...prev,
  ...
  const renderDropdown = (name: string, options: any[], label: string) => (
    ...
    <div className="flex flex-col">
      ...
      <div className={`grid ${gridCols} gap-2`}>
        {options.map((item: any) => {
          const isSelected = Array.isArray(filters[name]) && filters[name].includes(String(item.name));
          return (
            <label
              key={item.id}
              className={`
  ...
  const renderFilterForm = () => (
    ...
    >
      ...
      <div className="flex space-x-2">
        <button type="button" onClick={handleClearFilters} className={`${tailwindStyles.secondaryButton} w-full`}>
          Clear
        </button>
        <button type="submit" className={`${tailwindStyles.secondaryButton} w-full`}>
          Apply
        </button>
      </div>
    </form>
  ...
  return (
    <div className="w-full rounded-md bg-white h-auto md:h-full">
      <button className="md:hidden fixed left-2 top-14 z-20" onClick={() => setIsShow(!isShow)}>
      ...
      {isShow && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50">
          <div className="fixed top-0 left-0 sm:w-3/4 md:w-2/4 h-full bg-white shadow-lg z-50 p-4">
            <button className="absolute top-4 right-4 text-lg" onClick={() => setIsShow(false)}>
              ✕
            </button>
