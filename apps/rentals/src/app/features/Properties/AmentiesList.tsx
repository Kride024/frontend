...
import { fetchCommunityAmenities } from "../../shared/services/api/index";
...
const AmenitiesList: React.FC<AmenitiesListProps> = ({ communityId, isOpen = false }) => {
  const [amenities, setAmenities] = useState<AmenityCategory[]>([]);
  const [loading, setLoading] = useState(true);

  // Fetch amenities on mount / when communityId changes
  useEffect(() => {
    const getAmenities = async () => {
      try {
        if (communityId) {
          const response = await fetchCommunityAmenities(communityId);
          // The API wrapper might return { amenities: [...] }
          if (response && Array.isArray(response.amenities)) {
            setAmenities(response.amenities as AmenityCategory[]);
          } else {
            setAmenities([]);
          }
        } else {
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {amenities.map((categoryObj, index) => (
              <div key={index}>
                <p className="text-xs font-semibold text-gray-600 mb-1">{categoryObj.category}</p>
                <div className="flex flex-wrap gap-2">
                  {categoryObj.amenities.map((amenity, idx) => (
                    <div key={idx} className="flex items-center gap-1 bg-gray-100 px-2 py-1 rounded text-[9px] sm:text-xs">
