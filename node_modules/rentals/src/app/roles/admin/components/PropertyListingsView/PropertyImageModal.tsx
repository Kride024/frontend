...
interface ImageModalProps {
  ...
  handleDelete: (imageIndex: number) => void;
}

const ImageModal: React.FC<ImageModalProps> = ({ modalData, handleDelete }) => {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  if (!modalData.images || modalData.images.length === 0) {
    return <p className="text-center text-gray-500 mt-4">No images available.</p>;
  }
