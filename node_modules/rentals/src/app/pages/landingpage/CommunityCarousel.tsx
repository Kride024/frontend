...
const CommunityCarousel: React.FC = () => {
    ...
    const fetchCommunityImages = async () => {
      try {
        ...
        if (Array.isArray(res.data.data)) {
          setImages(res.data.data as CommunityImage[]);
        } else {
  ...
  const handleClick = (name: string) => {
    ...
    name = name.slice(6).trim().replace(/^myhome\s*/i, "");
  ...
  // -----------------------------
  if (loading) {
    return <p className="my-4 text-center text-gray-600">Loading carousel...</p>;
  }
