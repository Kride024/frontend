...
type SimilarPropertiesProps = {
  propertyType: string;
  similarCount: number | string;
};

const SimilarProperties: React.FC<SimilarPropertiesProps> = ({ propertyType, similarCount }) => {
  return (
    <div className="w-full">
      <div className="relative flex justify-between items-center bg-[#001433] px-6 py-2 rounded-t-3xl overflow-hidden transition-all duration-500 ease-out group">
        <div>
          <h2 className={`text-xl font-bold 2xl:text-lg relative text-left text-white tracking-tight z-10`}>
        ...
        <div>
          <img src="/BUILDING.png" className="w-14" alt="building" />
        </div>
      </div>
    </div>
  );
