...
// -----------------------------
const InfoItemSkeleton: React.FC<InfoItemSkeletonProps> = ({
  addBorderRight,
  addBorderBottom,
}) => (
  <div
...
// -----------------------------
const PropertyListingCardSkeleton: React.FC = () => {
  const totalItems = 8; // Matches detailEntries count
        ...
        <div className="mt-3 text-gray-800">
          <div className="grid grid-cols-2 lg:grid-cols-5 rounded-md border-4 border-gray-200">
            {Array.from({ length: totalItems }).map((_, index) => {
              const isSmallScreen = typeof window !== "undefined" ? window.innerWidth < 1023 : false;
              const totalColumns = isSmallScreen ? 2 : 4;
              const isLastColumn = (index + 1) % totalColumns === 0;
              const isLastRow =
                index >= totalItems - (totalItems % totalColumns || totalColumns);
...
// Wrapper Component
// -----------------------------
const PropertyListingSkeletonLoader: React.FC = () => {
  return (
