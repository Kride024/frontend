...
import useListingStore from "@/app/store/listingsStore";
...
import PropertyListingCardSkeletonLoader from "./PropertyCardSkelton";
...
interface UseListingStoreShape {
  apiResponse: ApiResponseShape;
  pagination: Pagination;
  currentPage: number;
  fetchListings: (filters: Record<string, any>, page?: number, limit?: number) => Promise<void>;
  setCurrentPage: (page: number) => void;
}

/** --- Components --- */
const InfiniteScrollLoader: React.FC<{ isVisible: boolean }> = ({ isVisible }) => (
  <div
...
const PropertiesView: React.FC = () => {
  // If your zustand store is typed properly, TypeScript will infer; otherwise cast:
  const {
    apiResponse,
    pagination,
    currentPage,
    fetchListings,
    setCurrentPage,
  } = useListingStore() as UseListingStoreShape;
  ...
  const getFilterDataFromParams = () => {
    return {
      ...
      propertydescription: searchParams.get("propertydescription")?.split(",") || [],
  ...
  const renderListings = () => {
    ...
    if (apiResponse.status === apiStatusConstants.inProgress) {
      return apiResponse.data.length === 0 ? (
        ...
        <>
          <SuccessView apiResponse={apiResponse} lastListingRef={lastListingRef} />
  ...
  useEffect(() => {
    const handleResize = () => {
      setMaxHeight(window.innerWidth < 450 ? "calc(100vh - 4rem)" : "calc(100vh - 6rem)");
    };
    handleResize();
        ...
        <div style={{ height: maxHeight }} className="grid md:grid-cols-10">
          ...
          >
            <FilterSection
              ...
              currentPageChange={handleFilterChange}
