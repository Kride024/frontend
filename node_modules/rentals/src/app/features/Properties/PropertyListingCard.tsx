...
// Format backend property data
const mapBackendDataToComponent = (property: PropertyDetails) => {
  ...
  return {
    ...
    communitydetails: `${property.total_area} Acres Campus with ${property.totflats} Units ${
      property.majorArea ? ", Near " + property.majorArea : ""
    }`,
    location: property.address,
    details: {
      "Rent/Month": `₹ ${property?.rental_high || "N/A"} ${
...
const PropertyListingCard: React.FC<PropertyListingCardProps> = ({ property }) => {
  ...
  const matchedProperty = userProperties.find(
    (userProp: any) => userProp.prop_id === property.id
  );

  const showRmNumber =
    matchedProperty?.rm_name
      ? {
          rmNumber: matchedProperty.RM_mobile_no || "RM Assigned",
  ...
  const connectToRM = async (
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setIsBrokeragePopupOpen(true), 200);
      }, 1500);
    } catch (error: any) {
    ...
    try {
      ...
      if (id) {
        ...
        if (result.status === 200) {
          ...
          setTimeout(() => setIsBrokeragePopupOpen(true), 400);
        }
      }
  ...
  const confirmRemoveFavourite = async (matched: any) => {
    try {
      const jwtToken = Cookies.get(jwtSecretKey);
      if (!jwtToken) {
        openModal();
    ...
    <>
      ...
      <div className="flex flex-col lg:flex-row gap-3 px-4 pt-4">
        ...
        <div className="space-y-2 relative">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 bg-gray-200 rounded-lg overflow-hidden">
