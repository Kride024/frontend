...
export default function Dashboard(): JSX.Element {
  ...
  const ARROW_SIZE = 36; // px

  const wrapperRef = useRef<HTMLDivElement | null>(null); // outer relative wrapper
  const containerRef = useRef<HTMLDivElement | null>(null); // inner padded container
  ...
  const [arrowOffset, setArrowOffset] = useState<number>(12); // distance from wrapper left/right
  const [visibleAreaWidth, setVisibleAreaWidth] = useState<number>(0); // width of centered card area
  ...
  const recomputeLayout = () => {
    ...
    let computedGap =
      gapsCount > 0
        ? Math.max(MIN_GAP, (remaining - preferredSidePadding * 2) / (gapsCount + 2))
        : Math.max(MIN_GAP, remaining / 2);

    if (!isFinite(computedGap) || computedGap <= 0) computedGap = MIN_GAP;

    const computedVisibleAreaWidth = Math.round(totalCardWidth + (maxCount - 1) * computedGap);
    ...
    // compute visible cards area width and arrow offset so arrows sit just outside cards
    const leftSpace = Math.max(0, (wrapperWidth - computedVisibleAreaWidth) / 2);
    // place arrows a bit outside the card area, but not off-screen
    const offset = Math.max(8, leftSpace - ARROW_SIZE / 2 - 6);
  ...
  // Cards (unchanged content, only kept inline for single-file paste)
  const OccupancyCard = (key?: string) => (
    <article
      key={key ?? "occupancy"}
    ...
    >
      ...
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center mt-3">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="164" height="159" viewBox="0 0 164 159" fill="none" className="w-full h-full donut">
  ...
  const ActiveGuestsCard = (key?: string) => (
    <article key={key ?? "active"} className="rounded-[30px] flex-shrink-0 active-guests-card" style={{ width: CARD_WIDTH, height: "487px", border: "1px solid rgba(79, 107, 227, 0.00)", background: "#FFF", boxShadow: "4px 4px 4px rgba(0, 0, 0, 0.10)" }}>
      <div
        className="h-full rounded-[20px] p-6 flex flex-col transition-transform duration-200"
        style={{
          display: "flex",
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <div className="flex gap-4 mt-7 items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold" style={{ fontFamily: "Poppins, sans-serif", fontSize: "14.4px", color: "#000" }}>
                Capacity
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold" style={{ fontFamily: "Poppins, sans-serif", fontSize: "14.6px", color: "#000" }}>
                Active Guest
              </span>
            </div>
          </div>
  ...
  const RevenueCard = (key?: string) => (
    <article key={key ?? "revenue"} className="rounded-[30px] flex-shrink-0 revenue-card" style={{ width: CARD_WIDTH, height: "487px", border: "1px solid rgba(79, 107, 227, 0.00)", background: "#FFF", boxShadow: "4px 4px 4px rgba(0, 0, 0, 0.10)" }}>
      <div
      ...
      >
        ...
        <div className="flex-1 flex items-start justify-center mt-6">
          <div className="relative w-full" style={{ maxWidth: 220 }}>
            ...
            >
              <div className="flex-1 flex flex-col items-start justify-start space-y-2" style={{ marginTop: "-37px" }}>
                <div>Profit</div>
                <div>Expense</div>
                <div>Revenue</div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center mt-7">
              <div className="font-semibold" style={{ fontFamily: "Poppins, sans-serif", fontSize: "14.4px", color: "#000" }}>
                INR in K
              </div>
            </div>
          </div>
  ...
  const OverdueCard = (key?: string) => (
    <article key={key ?? "overdue"} className="rounded-[30px] flex-shrink-0" style={{ width: CARD_WIDTH, height: "487px", border: "1px solid rgba(79, 107, 227, 0.00)", background: "#FFF", boxShadow: "4px 4px 4px rgba(0, 0, 0, 0.10)" }}>
      <div
      ...
      >
        ...
        <div className="mt-4 text-center" >
          ...
          <span className="font-Poppins text-black" style={{ fontWeight: 500, letterSpacing: "-0.154px", fontSize: "15px" }} >vs last month</span>
        </div>
  ...
  const cards = [OccupancyCard, ActiveGuestsCard, RevenueCard, OverdueCard];
  ...
  return (
    <div className="w-full p-4">
      ...
      <section className="flex justify-center relative ">
        <div className="w-full max-w-[1260px] mx-auto relative" ref={wrapperRef}>
          <div
            ref={containerRef}
            style={{
              overflowX: "hidden",
          ...
          >
            <div
              ...
              style={{
                ...
                width: visibleAreaWidth ? `${visibleAreaWidth}px` : "100%",
                margin: visibleAreaWidth ? "0 auto" : undefined,
                boxSizing: "border-box",
              }}
            >
              {cards
                .slice(startIndex, startIndex + visibleCount)
                .map((C, idx) => C(`card-${startIndex + idx}`))}
            </div>
          </div>
